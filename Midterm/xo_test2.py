...
while True:
    ...
    # -----------------------------
    # HUMAN TURN (with stability + wrong color)
    # -----------------------------
    if player_turn:
        if len(new_cells) == 0:
            pending_human_move = None
            ...
            if (row, col) not in expected_cells:
                ...
                if wrong_stable >= wrong_required:
                    print(
                        f"⚠️ Wrong color block used! You are playing {color_name}. "
    ...
    # -----------------------------
        ...
        if winner != 0:
            print(" Human wins!" if winner == human_symbol else " Robot wins!")
            winner_text = "Human wins!" if winner == human_symbol else "Robot wins!"
            end_time = time.time() + 3  # show message for 3 seconds
            while time.time() < end_time:
                ret, frame = cap.read()
                if not ret:
                    break
                cv2.putText(frame, winner_text, (60, 60), font, 1, (0, 255, 0), 2)
                cv2.imshow("Tic Tac Toe", frame)
