...
CALIB_IMG_PTS_PX_FULL = np.array(
    ...
    [534, 126],
    [531, 339],
    [105, 338],  # Camera point 4 (bottom-left)
], dtype=np.float32)
...
# print("[CALIBRATION] Loaded homography matrix from homography.npy")
...
def astar_path_finding(grid, start_grid, end_grid):
    h, w = grid.shape
    start_r, start_c = start_grid
    end_r, end_c = end_grid
    ...
    open_set = []
    heapq.heappush(open_set, (0 + heuristic(start_grid, end_grid), 0, start_grid))
        ...
        for dr, dc in [(-1,0),(1,0),(0,-1),(0,1)]:
            ...
            if 0 <= nr < h and 0 <= nc < w and grid[nr, nc] == 0:
                ...
                if tentative_g < g_score.get(neighbor, float('inf')):
                    ...
                    f = tentative_g + heuristic(neighbor, end_grid)
...
class GeminiAgent:
    ...
    def __init__(self, api_key="AIzaSyCWcOuOrD1euSG80-O90puFrOh08gQ-MLY"):
...
class MotionAgent:
    def __init__(self, port="/dev/ttyACM0"):
        ...
        self.ok = False
        from pydobot import Dobot
        self.dev = Dobot(port="/dev/ttyACM0")
        self.ok = True
...
def main():
    ...
    print("[INFO] Uses Gemini AI for marker detection and A* for path finding")
    print("[SOLVE] Solving maze with wall detection and A*...")
    ...
    while True:
        ...
        if key == ord(' '):  # SPACE - Detect markers with AI
            ...
            if start_pos and end_pos:
                ...
                grid_path = astar_path_finding(maze_grid, start_grid, end_grid)
